...
`.trim()
     	try {
        if (/amnhop|tokohewan/i.test(command)) {
        ...
        if (DevMode) {
            for (let jid of global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != conn.user.jid)) {
                conn.sendMessage(jid, 'anmshop.js error\nNo: *' + m.sender.split`@`[0] + '*\nCommand: *' + m.text + '*\n\n*' + e + '*', MessageType.text)
              
            }
        }
    }
  }
handler.help = ['anmhop <sell|buy>  <item> <args>', 'tokohewan <sell|buy> <item> <args>']
handler.tags = ['rpg']

handler.command = /^(anmshop|tokohewan)$/i
